...
# Docker镜像构建脚本（仅本地构建 server，不推送）
# 使用方法: ./build.sh  或  ./build-server.sh
...
# 检查是否在项目根目录
if [ ! -f "Dockerfile-server" ] || [ ! -f "Dockerfile-web" ]; then
...
echo -e "${YELLOW}说明: 本脚本现在只负责本地构建 server 镜像，不会执行登录和推送操作。${NC}"

# 构建服务端镜像（Python 服务）
echo ""
echo -e "${GREEN}========================================${NC}"
echo -e "${YELLOW}步骤1/1: 构建服务端镜像...${NC}"
echo -e "${GREEN}========================================${NC}"
echo "开始构建服务端镜像..."
...
echo ""
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}server 镜像已在本地构建完成（未推送）！${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
