...
('SYSTEM_TTS_DogLanguage', 'TTS', 'dog_language', '犬语', '[{"key":"dog_sounds_dir","label":"犬叫声文件夹路径","type":"string"},{"key":"default_sound_type","label":"默认犬叫声类型","type":"string"}]', 2, 1, NOW(), 1, NOW());

-- 添加犬语TTS模型配置
delete from `ai_model_config` where id = 'TTS_DogLanguage';
INSERT INTO `ai_model_config` VALUES ('TTS_DogLanguage', 'TTS', 'DogLanguage', '犬语', 0, 1, '{\"type\": \"dog_language\", \"dog_sounds_dir\": \"config/dog_sounds\", \"default_sound_type\": \"01_positive_greeting\"}', NULL, NULL, 2, NULL, NULL, NULL, NULL);
...
2. 需要在服务器上创建犬叫声文件夹，结构如下（16种类型，按4大类分类）：
   config/dog_sounds/
